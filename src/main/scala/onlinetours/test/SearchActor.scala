...
class SearchActor(request: Request, requester: ActorRef, db: ActorRef, tourDb: ActorRef, api: ActorRef) extends Actor {
    ...
    var lastCheapestTourId: String = null   //последний самый дешевый - нужен для того, чтобы не запускать несколько раз поиск перелётов для одинаковых туров
    var bestTourCost: Int = Int.MaxValue
    ...
    def receive = {
        ...
        case Cheapest(tour) =>
            tour match {
                case Some(dbTour) =>
                    if (!dbTour.tourId.equals(lastCheapestTourId) && (resultTour == null || dbTour.minCost < resultTour.minCost)) {        //нашли новый тур - первый или более дешевый тур
                        lastCheapestTourId = dbTour.tourId
                        if (dbTour.minCost < bestTourCost) bestTourCost = dbTour.minCost
                        ...
                        if (searchCompleted && resultTour.minCost == bestTourCost)          //если поиск в БД закончился и мы уже нашли лучший результат
        ...
        case TourFetched(tour) =>
            tour match {
                case Some(dbTour) =>
                    if (resultTour == null || dbTour.minCost < resultTour.minCost) {       //если результата нет или тур дешевле текущего результата
                        if (!dbTour.hasFlightsInformation)          //если информация ещё не найдена - ждём
                            ...
                            //информация найдена - сохраняем/заменяем результат
                            resultTour = dbTour
                            if (searchCompleted && resultTour.minCost == bestTourCost)            //если поиск в БД закончился и мы уже нашли лучший результат
                                self ! ReturnSearchResult()
                        }
